...
: _name(name), _lat(lat), _lon(lon), _coords(lat,lon)
{
    pair<float,float> coords = geoToLambert93(lat,lon);
    _coords = Point2D(coords.first, coords.second);
}
...
{
    vector<Town> found;
    for(vector<Town>::const_iterator it=towns.begin(); it!=towns.end(); ++it)
        if (it->name() == name)
            found.push_back(*it);
    return found;
}
...
ostream &operator<<(ostream &os, Town const &town) {
    return os << town.name() << "(" << town.lat() << "," << town.lon() << ")";
}
...
bool Town::operator==(const Town &town) const {
    return _name == town._name && _coords == town._coords;
}
bool Town::operator!=(const Town &town) const {
    return !(operator==(town));
}
bool Town::operator<(const Town &town) const {
    return (_coords < town._coords ||
            (_coords == town._coords && _name < town._name));
}
...
namespace std
{
    template<> struct hash<Town>
    {
        size_t operator()(Town const& town) const {
            size_t const h0 (std::hash<std::string>()(town.name()) );
            size_t const h1 (std::hash<float>()(town.x()));
            size_t const h2 (std::hash<float>()(town.y()));
            return h0 ^ (h1 << 1) ^ (h2 << 2);
        }
    };
}
